...
HOST = r"192.168.0.32"
PORT = 9999
PBS_DIR = r"C:\Users\wynta1.209\Downloads\Transfer\Pokemon\AstralServ"
...
def make_party_validator(pbs_dir):
    ...
    with io.open(os.path.join(pbs_dir, r'items.txt'), 'r', encoding='utf-8-sig') as items_pbs:
        for row in csv.reader(items_pbs):
            if len(row) >= 1:
                thinggameing = int(row[0])
                item_ids.add(thinggameing)
    ...
    with io.open(os.path.join(pbs_dir, r'pokemon.txt'), 'r', encoding='utf-8-sig') as pokemon_pbs:
        ...
        for number in pokemon_pbs_.sections():
            ...
            if not (moves_ := form_moves_by_name.get(species['InternalName'])):
                moves_ = species['Moves']
        ...
        for line in tm_pbs:
            ...
            if line.startswith('#'):
                ...
                for name in line.split(','):
                    if name:
                        pokemon_by_name[name].moves.add(move)
